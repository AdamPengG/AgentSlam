...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Office + Nova Carter with Isaac Sim's native ROS2 front-stereo publishers."
    )
    parser.add_argument(
        "--scene",
        default="/home/peng/isaacsim_assets/Assets/Isaac/5.1/Isaac/Environments/Office/office.usd",
        help="Absolute path to the office scene USD.",
    )
    parser.add_argument(
        "--robot",
        default="/home/peng/isaacsim_assets/Assets/Isaac/5.1/Isaac/Samples/ROS2/Robots/Nova_Carter_ROS.usd",
        help="Absolute path to the ROS-enabled Nova Carter USD.",
    )
    parser.add_argument(
        "--robot-prim-path",
        default="/World/NovaCarter",
        help="Prim path where the Nova robot will be referenced into the scene.",
    )
    parser.add_argument(
        "--camera-namespace",
        default="front_stereo_camera",
        help="Namespace prefix used by the native front_hawk ROS graph.",
    )
    parser.add_argument(
        "--headless",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Disable viewport updates while headless. Keep this off when validating camera throughput.",
    )
    parser.add_argument(
        "--warmup-frames",
        type=int,
        default=60,
        help="App-update frames after loading the robot reference but before enabling publishers.",
    )
    parser.add_argument(
        "--publish-without-verification",
        action="store_true",
        default=True,
        help="Publish camera data even when ROS subscribers are not connected yet.",
    )
    parser.add_argument(
        "--checkerboard",
        default="",
        help="Optional checkerboard USD to spawn for reprojection audits.",
    )
    ...
    parser.add_argument("--board-meta-json", default="")
...
def main() -> int:
    ...
    app = SimulationApp(
        {
            ...
            "disable_viewport_updates": bool(args.disable_viewport_updates),
        }
    )
    ...
    try:
        ...
        if args.checkerboard:
            ...
            add_reference_to_stage(checkerboard_path, str(args.checkerboard_prim_path))
        ...
        if args.checkerboard and args.board_meta_json:
            ...
            chassis_orientation = np.asarray(chassis_orientation, dtype=np.float64)
            ...
            board_world_quat = chassis_quat * Gf.Quatd(float(board_local_quat[3]), Gf.Vec3d(*board_local_quat[:3]))
            ...
            payload = {
                ...
                "board_square_cols": int(args.board_square_cols),
                "board_square_rows": int(args.board_square_rows),
                "board_inner_cols": int(args.board_square_cols - 1),
                "board_inner_rows": int(args.board_square_rows - 1),
            }
